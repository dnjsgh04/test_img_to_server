...
//이미지 서버에 올리기 https://ojlog.tistory.com/4
...
class ImageUploadState extends State<ImageUpload> {
                ...
                Row(
                  children: [
                    ...
                    Container(
                        ...
                        child: IconButton(
                            onPressed: () async {
                              oneImage = await picker.pickMedia();
  ...
  Future<void> oneImageSend() async {
    // dio.options.contentType = 'multipart/form-data';
    // dio.options.maxRedirects.isFinite;

    final formData = FormData.fromMap({
      'name': 'dio',
      'image' : await MultipartFile.fromFile(oneImage!.path, filename:oneImage!.name),
    });
    final response = await dio.post('http://172.30.1.46:8080/imgfile/upload', data: formData);

    print(response.data.toString());
  }

  Future<void> multiImageSend() async {
    dio.options.contentType = 'multipart/form-data';
    dio.options.maxRedirects.isFinite;

    final formData = FormData.fromMap({
      'name': 'dio',
      //'image' : await MultipartFile.fromFile(multiImage., filename:oneImage!.name),
    });

    for (var item in multiImage)
      formData.files.addAll([
        MapEntry("image_files", await MultipartFile.fromFile(item!.path)),
      ]);
    final response = await dio.patch('http://172.30.1.46:8080/imgfile/multiUpload', data: formData);
